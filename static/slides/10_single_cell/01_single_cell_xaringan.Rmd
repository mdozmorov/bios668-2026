...
* **The Dropout Model**: ZIFA assumes that the probability of a dropout event ($p_0$) is a decreasing function of the latent expression level ($\mu$). It uses a decaying exponential relationship:
  $$p_{0}(\text{dropout} | \mu) = \exp(-\lambda \mu^2)$$
...
* $\delta(c_i, c_j)$ is the Kronecker delta function, $1$ if $i$ and $j$ are in the same community, and $0$ otherwise.
...
Here’s a clean, slide-ready way to present **Leiden** with its underlying objective and how it relates to modularity.

---
## Graph-Based Clustering: The Leiden Objective Function

Leiden optimizes a **quality function** similar to modularity, but more general:

$$Q = \sum_{i,j} \left[ A_{ij} - \gamma , n_i n_j \right] \delta(c_i, c_j)$$

* $A_{ij}$: edge weight between nodes (i) and (j) (e.g., SNN Jaccard weight)
* $n_i, n_j$: node weights (often degree-based; in modularity $n_i = \frac{k_i}{2m})$
     * $\gamma$: **resolution parameter** (controls cluster granularity)
...
2.  **Graph-Autocorrelation (`graph_test`)**: Uses **Moran's I** statistic (spatial autocorrelation) to find genes that vary "smoothly" across the trajectory graph.  
...
---
## MAST: assessing transcriptional changes and characterizing heterogeneity in scRNA-seq data
...
* *Usually modeled on normalized values.*
